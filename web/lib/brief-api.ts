...
export type Brief={summary_pending?:number;summary_failed?:number;stories:Story[];query:string;country:string;country_name:string;generated_at:number;source_updated_at:number;cache:string;stale:boolean;intent:Record<string,unknown>;matched_articles:number;notice:string|null;selected_sources:number;requested_count:number};
