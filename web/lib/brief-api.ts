...
export type Story={id:string;title:string;summary:string;summary_kind:string;topic:string;country?:string;country_name?:string;local_priority?:boolean;published_at:number;coverage_count:number;saved:boolean;sources:{id:string;url:string;publisher:string;excerpt?:string;link_kind?:string;domain?:string;major?:boolean;title:string;published_at:number}[];verification:{status:string;major_count:number;total_count:number;major_outlets:{id:string;name:string;domain:string}[];checked_at:number;expires_at:number;explanation:string;stale?:boolean}};
export type Brief={stories:Story[];query:string;country:string;country_name:string;generated_at:number;source_updated_at:number;cache:string;stale:boolean;intent:Record<string,unknown>;matched_articles:number;notice:string|null;selected_sources:number;requested_count:number};
export type Status={groq:string;groq_configured:boolean;groq_model:string;key_location:string;semantic_cache:string;country:string|null;country_name:string|null;articles:number;cached_queries:number;refreshing:boolean;last_fetch:number};
export async function api<T>(path:string,options:RequestInit={}):Promise<T>{
 ...
 try{const r=await fetch(`/api/${path}`,{...options,signal});if(!r.ok){const body=await r.json().catch(()=>({})) as {detail?:unknown};throw new Error(typeof body.detail==='string'?body.detail:`Request failed (${r.status}). Try again.`)}return r.json()}
 catch(e){if(controller.signal.aborted)throw new Error('This took too long. Please retry; your cached stories are safe.');if(e instanceof TypeError)throw new Error('Cannot reach the local news service. Run start.ps1 and keep its terminal open.');throw e}
